  ...
  <div class="bg-[#f8fafc] dark:bg-background min-h-screen flex flex-col font-sans">
    <header class="flex items-center justify-between whitespace-nowrap border-b border-solid border-border px-6 py-3 bg-white dark:bg-card">
      ...
      <div class="flex items-center gap-4">
        <router-link 
          to="/terms"
          class="text-sm text-primary hover:underline font-medium hidden sm:block"
        >
          Need help?
        </router-link>
        <router-link 
          to="/signup"
          class="flex min-w-[84px] cursor-pointer items-center justify-center rounded-lg h-10 px-4 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-bold transition-colors"
        >
          Sign up
        </router-link>
      </div>
