...
export class TechnicalTemplate {
  /**
  * Generates a PDF buffer from resume data using the Technical template style
  * Engineering-focused layout with technical section labels
  ...
  async generate(resume) {
    ...
    if (model.experience.length > 0) {
      ...
      for (const job of model.experience) {
        ...
        y = addTwoColumnRow(roleLine, job.dateLine, y, {
          leftFont: fontBold,
          leftSize: fontSize,
    ...
    // SKILLS
    if (model.skills.text || model.skills.tags.length > 0) {
      drawSectionHeader('Technical Skills');
      const skillCategories = resume?.sections?.skillCategories;
      if (skillCategories && typeof skillCategories === 'object') {
        const categoryRows = [
          {
            title: skillCategories.languages?.title || 'Languages',
            skills: Array.isArray(skillCategories.languages?.skills) ? skillCategories.languages.skills : []
          },
          {
            title: skillCategories.frameworks?.title || 'Frameworks',
            skills: Array.isArray(skillCategories.frameworks?.skills) ? skillCategories.frameworks.skills : []
          },
          {
            title: skillCategories.tools?.title || 'Tools & Infra',
            skills: Array.isArray(skillCategories.tools?.skills) ? skillCategories.tools.skills : []
          }
        ...
        if (categoryRows.length > 0) {
          for (const row of categoryRows) {
            y = checkPageBreak(y);
            y = addText(`${row.title}:`, margin, y, { font: fontBold, color: accentColor, align: 'left' });
            y = addText(row.skills.join(', '), margin, y, { align: bodyTextAlign });
            y -= 3;
          }
        } else if (model.skills.tags.length > 0) {
